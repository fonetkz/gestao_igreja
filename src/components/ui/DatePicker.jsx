...
const DatePicker = ({ value, onChange, label, error }) => {
  ...
  const [dropdownPos, setDropdownPos] = useState({ left: 0, top: 0, width: 256 })
  ...
  return (
    <div className="flex flex-col w-full" ref={wrapperRef}>
      ...
      {isOpen && createPortal(
        <div
          ref={dropdownElRef}
          className="fixed z-[100] w-64 bg-white dark:bg-gray-800 rounded-2xl shadow-high border border-gray-200 dark:border-gray-500 overflow-hidden animate-scale-in"
          style={{ left: dropdownPos.left, top: dropdownPos.top, width: dropdownPos.width }}
        >
          <div className="p-3 border-b border-gray-100 dark:border-gray-500">
            <div className="flex items-center justify-between">
              ...
              <span className="font-bold text-sm text-gray-900 dark:text-white min-w-[100px] text-center">
          ...
          <div className="p-2">
            <div className="grid grid-cols-7 gap-0.5 mb-1">
              {dayNames.map((day, i) => (
                <div key={i} className="text-center text-[9px] font-semibold text-gray-400 py-1">
                  {day}
                </div>
              ))}
            </div>
          ...
          <div className="p-2 border-t border-gray-100 dark:border-gray-500 bg-gray-50 dark:bg-gray-800/50 flex gap-2">
            <button
              ...
              className="flex-1 py-1.5 text-xs font-medium text-primary dark:text-blue-300 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded transition-colors"
            >
              Hoje
            </button>
            <button
              type="button"
              onClick={handleClear}
              className="flex-1 py-1.5 text-xs font-medium text-gray-500 hover:text-gray-700 dark:hover:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 rounded transition-colors"
            >
              Limpar
            </button>
          </div>
